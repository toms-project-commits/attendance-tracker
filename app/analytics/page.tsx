      ...
      <div className="max-w-3xl mx-auto p-4 md:p-8 space-y-6">
          ...
          <div className="flex flex-col md:flex-row items-center justify-between gap-8 relative z-10">
            <div className="text-center md:text-left space-y-4">
              ...
              <div className="inline-flex gap-4 bg-slate-800/80 backdrop-blur-sm p-3 rounded-xl text-sm border border-slate-700/50 shadow-inner">
                <div className="flex flex-col items-center px-2">
                  <span className="text-green-400 font-bold text-lg">{analyticsData.overall.attended}</span>
                  <span className="text-[10px] text-slate-400 uppercase font-bold">Present</span>
                </div>
                <div className="w-px bg-slate-700"></div>
                <div className="flex flex-col items-center px-2">
                  <span className="text-red-400 font-bold text-lg">{analyticsData.overall.total - analyticsData.overall.attended}</span>
            ...
            <div className="relative w-36 h-36 shrink-0">
              <svg viewBox="0 0 36 36" className="w-full h-full rotate-[-90deg]">
                ...
                <path className={clsx("transition-all duration-1000 ease-out", analyticsData.overall.percentage >= 75 ? "text-green-500" : "text-red-500")} strokeDasharray={`${analyticsData.overall.percentage}, 100`} d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" />
              </svg>
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <h2 className="text-lg font-bold text-slate-800">Subject Breakdown</h2>
            <div className="flex gap-2">
              <button
                onClick={() => setSortBy('status')}
                className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-colors ${sortBy === 'status' ? 'bg-blue-600 text-white' : 'bg-white text-slate-600 border border-slate-200 hover:bg-slate-50'}`}
              >
                Status
              </button>
              <button
                onClick={() => setSortBy('percentage')}
                className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-colors ${sortBy === 'percentage' ? 'bg-blue-600 text-white' : 'bg-white text-slate-600 border border-slate-200 hover:bg-slate-50'}`}
              >
                %
              </button>
              <button
                onClick={() => setSortBy('name')}
                className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-colors ${sortBy === 'name' ? 'bg-blue-600 text-white' : 'bg-white text-slate-600 border border-slate-200 hover:bg-slate-50'}`}
              >
                A-Z
              </button>
            </div>
            ...
            {sortedStats.map((sub) => (
              <div key={sub.id} className="bg-white p-5 rounded-2xl shadow-sm border border-slate-100">
                <div className="flex justify-between items-start mb-4">
                  ...
                  <Link href="/mark" className="p-2 hover:bg-slate-100 rounded-lg transition-colors" title="Edit attendance">
